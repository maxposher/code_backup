...
WHERE date(pstday1) > '2016-01-01' --Max's update
...
from
 ...
 , source
 , source_group
 , CASE WHEN ((CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) in ('rip'))  THEN 'Referral' 
        WHEN ((reg_app in ('iphone', 'android', 'ipad') or reg_app is NULL) and (((CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) in ('cwgt','dt','fb','February+2014','gks','gs','gshm','homepage', 'home page', 'na','ntr','ot','poshmark','pulsenews','pvrl','SavingsMania+Newsletter','SPRING+POP+UP','tm','tw','unknown','ext_fb', 'ext_tm', 'ext_tw', 'ext_pi', 'ext_gp', 'invitefriends-email', 'invitefriends-fb','invitefriends-sms','invitefriends-tu', 'invitefriends-tw', 'organic')) OR ((CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) IS NULL )))  THEN 'Mobile_Organic' 
        WHEN ((reg_app in ('iphone', 'android', 'ipad') or reg_app is NULL) and ((CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) not in ('rip','cwgt','dt','fb','February+2014','gks','gs','gshm','homepage','na','ntr','ot','poshmark','pulsenews','pvrl','SavingsMania+Newsletter','SPRING+POP+UP','tm','tw','unknown','ext_fb', 'ext_tm', 'ext_tw', 'ext_pi', 'ext_gp') and ((CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) IS NOT NULL)))  THEN 'Mobile_Paid' 
        WHEN (reg_app in ('web') and (((CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) in ('pi_sh_app', 'pi_sh_pub','tm_sh_pub','organic','em_sh_pub','ext_pn','fb_sh_msn' , 'fb_sh_mshn','fb_sh_pub', 'invitefriendsemail', 'invitefriendsms', 'invitefriendtu', 'invitefriendtw', 'cwgt','dt','fb','February+2014','gks','gs','gshm','homepage','na','ntr','ot','poshmark','pulsenews','pvrl','SavingsMania+Newsletter','SPRING+POP+UP','tm','tw','unknown','ext_fb', 'ext_tm', 'ext_tw', 'ext_pi', 'ext_gp', 'invfriendfb')) OR ((CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) IS NULL )))  THEN 'Web_Organic' 
        WHEN (reg_app in ('web') and (CASE WHEN (lhs.pstday < '2014-10-01' and acq_channel_lc not in ('rip')) THEN (CASE WHEN (heuristical_acq_channel is NULL) THEN 'organic' WHEN (heuristical_acq_channel = 'other') THEN 'other_paid' ELSE heuristical_acq_channel END) WHEN acq_channel_lc IN ('rip') OR is_referred_with_code in ('Yes') THEN 'rip' ELSE acq_channel_lc END) in ('gdm','fb_dpa', 'bdm', 'fbdm', 'shopzila','shopzilla', 'ydm'))  THEN 'Web_Paid_GDM' 
        ELSE 'Web_Paid' 
        END AS "acq_channel_super_group"
 ...
 , sum(gmv) as GMV
from
 ((select distinct user_id as device_id , reg_app as platform,joined_at::date as pstday, heuristical_acq_channel, is_referred_with_code, reg_app,
  case when acq_channel_lc='adaction interactive' then 'adaction interactive'
       when acq_channel_lc='facebook' then 'facebook'
       when acq_channel_lc='instagram' then 'instagram'
       when acq_channel_lc='pin_cpc' then 'pinterest'
...
(select buyer_id as device_id, booked_at::date as pstday, app as platform,
 ...
 AND dw_orders.order_state IN ('ab_captured', 'ab_captured_skipped', 'buyer_confirm_initiated',
                                ...
                                'seller_notification_initiated', 'waiting_seller_kyc_info', 'waiting_seller_zip',
                                 ...
                                 'seller_ab_updated', 'seller_ab_update_failed', 'waiting_seller_kyc_verification')
group by 1,2,3) as rhs
using ( device_id)) group by 1,2,3,4,5,6,7) AS e
